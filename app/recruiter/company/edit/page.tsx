...
export default function EditCompanyPage() {
      ...
      try {
        ...
        if (company) {
          ...
          if (company.pictures && company.pictures.length > 0) {
            ...
            setPicturePreviews(company.pictures);
          }
  ...
  const removePicture = (index: number) => {
    ...
    if (index < existingPictures.length) {
      ...
      const newExisting = existingPictures.filter((_, i) => i !== index);
      setExistingPictures(newExisting);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload pictures (if any new ones)
      ...
      if (logoPreview && logoPreview !== existingLogo) URL.revokeObjectURL(logoPreview);
      picturePreviews.forEach((url, index) => {
        if (index >= existingPictures.length) {
